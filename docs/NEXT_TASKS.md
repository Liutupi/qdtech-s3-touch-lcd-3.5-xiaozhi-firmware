# 2026-08-12 v1.8.23 发布后验收

- 用一块仍在 `v1.8.22` 的其他板子执行正常 OTA，确认能发现并安装 `v1.8.23`，重启后版本号正确且 NVS、SD 卡、主题和双模式入口均保留。
- 在客户曾经出现“能看到 Xiaozhi、提交路由器后无法联网”的路由环境中复测一次，记录原始 Station 身份与兼容身份的关联结果；不要为制造失败而修改路由器或擦除正常用户数据。
- OTA 后回归天气 HTTP 200、时间同步、MQTT、小智对话、2048 长时间运行、MD 游戏启动/退出和休眠唤醒。

# 2026-08-05 v1.8.18 立体骰子与摇一摇全屏化发布后验收

- 用另一块 v1.8.17 设备执行正常 OTA，确认能发现并安装 v1.8.18，重启后显示版本 1.8.18。
- 解压 v1.8.18 完整 SD 卡包到卡根目录，确认 `shake_lab/dice/stage.rgb565`、`roll.argb8888` 和 `land.argb8888` 存在，同时保留 `movies`、`books` 等旧资源。
- 分别选择 1、3、6 颗骰子，各摇至少 10 次；每次都应快速起摇、自动停下、只结算一次，结果仅为 1–6 点且不能卡在滚动途中。
- 检查所有“摇一摇实验室”子页面均使用完整屏幕空间；摇摇电影和摇摇书籍仍保持大封面、完整介绍与正常滑动。
- 反复进入和退出骰子页后，回归天气、语音、电台、日历、掌卦、功德木鱼和游戏馆；PSRAM 不应持续下降。本版本未加入新的 Wi-Fi 修复。
- 回退只需 App-only 刷回 v1.8.17，不擦除 NVS；SD 卡资源可保留，旧固件会忽略新增骰子目录。

# 2026-08-05 v1.8.17 摇摇电影与书籍发布后验收

- 用另一块 v1.8.16 设备执行正常 OTA，确认能发现并安装 v1.8.17，重启后显示版本 1.8.17。
- SD 卡根目录确认存在 `movies` 与 `books`；分别进入“摇摇电影”和“摇摇书籍”，完成至少 10 次推荐。
- 检查左侧封面大小与清晰度；右侧上下滑动到简介末尾，确认标题、评分、主创/作者、年份和完整简介均可阅读。
- 反复进入/退出两个页面，确认封面切换、返回按钮和再次摇动正常，PSRAM 不持续下降。
- 回归检查功德木鱼、掌卦、骰子、天气、小智语音、电台、日历与游戏馆；本版本没有加入新的 Wi‑Fi 修复。
- 回退仅刷 v1.8.16 App，不擦除 NVS；SD 卡资源可以保留。

# 2026-07-25 v1.8.6 多板 OTA 与电台目录验收

- 使用一块 v1.8.5 板子通过正常 OTA 发现并安装 v1.8.6，确认重启后版本正确。
- 插入 v1.8.6 SD 包后，进入电台 -> Catalog，确认五个中文非空分类、中文台名、直选和分类内上一台/下一台。
- 每个分类至少播放一台 30 秒，并验证广东音乐之声；确认小智音频焦点暂停/恢复、Wi-Fi、天气与内存没有回归。
- 回滚（保留 NVS 和 SD）：将 v1.8.5 App-only 镜像写入两个 OTA 槽 0x100000 与 0x800000；不得擦除 Flash 或重写 NVS。
...
# 2026-08-04 v1.8.16 发布后验收

- 用另一块 v1.8.15 设备通过正常 OTA 检查并安装 v1.8.16，确认重启后版本为 1.8.16。
- 确认 SD 卡根目录存在 `wooden_fish/background.rgb565`，进入“摇一摇实验室” -> “功德木鱼”。
- 分别触摸一次和轻敲机身一次，确认每次只增加 1 功德、动画完整且短木鱼音播放正常。
- 连续体验 10 次后退出页面，确认小智语音、音乐/电台、手机配置页与 Wi-Fi 均正常。
- 回退仅刷 v1.8.15 App；不得擦除 Flash 或重写 NVS。
# 2026-08-08 v1.8.19 世嘉双模式发布后验收

- 用一块未安装 `mdemu` 分区的 v1.8.18 板执行普通 OTA，确认发现并安装 v1.8.19；重启后版本正确、原功能正常、MD 入口自动隐藏。
- 用一块已完成双模式完整刷机的板执行普通 OTA，确认主 App 更新后仍可选择 MD 游戏、自动存档、安全退出并恢复联网。
- 核对 OTA 实际下载 `qdtech-s3-touch-lcd-3.5-v1.8.19-app.bin`，并通过 Release 中的 `SHA256SUMS.txt` 校验；不得把 full 镜像或 `mdemu.bin` 用作普通 OTA。
- 当前版本只远程更新主 App。旧板首次增加 `mdemu` 仍需 USB 完整刷机；模拟器独立 OTA 留作后续单独设计，不与本次发布混合。
# 2026-08-10 v1.8.20 暖色主题视觉升级发布后验收

- 用一块 v1.8.19 板通过正常 OTA 发现并安装 v1.8.20，确认实际下载 `qdtech-s3-touch-lcd-3.5-v1.8.20-app.bin`。
- 选择暖色主题，检查主页左上角小智头像、Wi-Fi 旁旋转地球和“今日一句”右侧小智插画均存在且不重叠。
- 检查应用中心卡片、更多/返回按钮和选中状态，确认浅色背景下所有中文均清晰可读。
- 进入小智页，依次回归待机、聆听、说话及其他状态动画；确认画面流畅、没有异常内存下降。
- 回归 Wi-Fi、天气、时间、电台、日历、摇一摇实验室、益智游戏馆和 MD 启动/退出；本版没有修改这些功能逻辑。
- 回退仅刷 v1.8.19 App，不擦除 NVS、SD 卡或 `mdemu` 分区。

# 2026-08-10 v1.8.21 2048 稳定性修复发布后验收

- 用一块 v1.8.20 板通过正常 OTA 发现并安装 v1.8.21，确认实际下载 `qdtech-s3-touch-lcd-3.5-v1.8.21-app.bin`。
- 联网并保持天气和 MQTT 正常运行，连续玩 2048 至少 15–20 分钟或完成 500 次操作，确认无卡顿、死机、看门狗复位或异常重启。
- 对着无法移动的边界方向连续点击，确认不再触发无效整板重绘；检查数字、方块配色、得分、最大方块、重新开始和退出均正常。
- 回归其他益智游戏、Wi-Fi、天气、语音、电台、日历、摇一摇实验室、SD 卡和 MD 启动/退出。
- 回退仅刷 v1.8.20 App，不擦除 NVS、SD 卡或 `mdemu` 分区。
