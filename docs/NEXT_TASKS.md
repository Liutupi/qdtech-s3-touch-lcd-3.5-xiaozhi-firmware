...
- Firmware `v1.7.19` is the current higher-version release target after that bootstrap. It includes real QDTech battery ADC reporting and BOOT/GPIO0 deep-sleep soft-power code, and publishes the required standalone `*-app.bin` asset.
...
- Verify a full board-initiated OTA from `1.7.17` or older to `1.7.19`: check -> update -> download -> partition write -> reboot -> new version.`r`n- Verify BOOT physical-key wiring: watch for `BOOT gpio down`; if absent, identify the actual key GPIO or accept that GPIO0 BOOT is not available as a runtime power button on this board.
