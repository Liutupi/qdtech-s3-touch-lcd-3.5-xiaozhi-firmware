...
- 时间同步和天气显示。
- 可选日历称骨入口：支持 1901-2100 公历出生日期与时辰选择，从 SD 卡读取歌诀及六页现代文化解读。
- 可选日历星座入口：支持 1900-2100 公历出生日期、十二星座推算、六页 SD 卡详情及十二张按需加载的可爱星座图片。
- 网络电台 MP3 播放。
...
- 构建时启用 `CONFIG_QDTECH_EXPERIMENT_CALENDAR_BONE_WEIGHT=y`；该实验开关默认关闭。
- 把发布资产中的 SD 数据包解压，将顶层 `calendar` 文件夹复制到 SD 卡根目录。
- 最终数据路径为 `/calendar/bone_weight/`。
- 进入 Calendar，点击 `称骨`，选择公历出生年月日和小时后开始推算。
- 详细阅读页提供歌诀、总体、性格与处事、事业与财务、人际与情感、生活建议六页，可用按钮或左右滑动翻页。
- 数据仅作传统民俗文化娱乐参考；现代解读由本项目整理，不构成健康、财务或人生决策建议。

## 如何使用日历星座阅读

- 构建时显式启用 `CONFIG_QDTECH_EXPERIMENT_CALENDAR_ZODIAC`；源码默认关闭。
- 把交付包中的 `calendar/zodiac` 目录复制到 FAT 格式 SD 卡根目录，保留已有的 `calendar/bone_weight` 目录。
- 进入 Calendar，点击 `星座`，输入出生年月日即可推算星座。
- 结果页显示星座、日期范围、元素、模式和守护星；点击 `详细解读` 后可用按钮或左右滑动阅读六页内容。
- 图片路径为 `/calendar/zodiac/images/<sign_id>.jpg`，推荐 160x160 RGB JPEG，单文件不超过 256 KiB。
